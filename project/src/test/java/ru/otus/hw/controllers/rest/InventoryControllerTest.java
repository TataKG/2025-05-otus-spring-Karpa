...
class InventoryControllerTest {
    ...
    @Test
    @DisplayName("Создание инвентаря - успешный случай")
    @WithMockUser
    void createInventory_ShouldCreateInventory_WhenValidRequest() throws Exception {
        // Arrange
        ...
        when(inventoryService.createInventory(eq(INVENTORY_NAME), eq(INVENTORY_DESCRIPTION)))
                .thenReturn(testInventoryDto);
        ...
        mockMvc.perform(post("/api/inventories")
                ...
                .andExpect(jsonPath("$.message").value(INVENTORY_CREATED_MESSAGE));
    }

    @Test
    @DisplayName("Создание инвентаря - конфликт при существующем имени")
    @WithMockUser
    void createInventory_ShouldReturnConflict_WhenInventoryAlreadyExists() throws Exception {
        // Arrange
        String requestBody = String.format(
                "{\"name\": \"%s\", \"description\": \"%s\"}",
                INVENTORY_NAME, INVENTORY_DESCRIPTION
        );
        String errorMessage = "Инвентарь с таким именем уже существует";

        when(inventoryService.createInventory(anyString(), anyString()))
    ...
    @Test
    @DisplayName("Создание инвентаря - неверный запрос")
    @WithMockUser
    void createInventory_ShouldReturnBadRequest_WhenInvalidRequest() throws Exception {
        // Arrange
        String requestBody = "{\"name\": \"\", \"description\": \"\"}";
        String errorMessage = "Название инвентаря не может быть пустым";
